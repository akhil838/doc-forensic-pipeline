...
\newcommand{\contactemail}{akhil838@users.noreply.github.com}
...
OpenCV-detected portrait crops across all five document types, achieving 99.4\%
validation accuracy.
...
(commit \texttt{c3a7d9ff})\\
\textbf{Weights:} \url{https://huggingface.co/akhil838/doc-forensic-models-v1} (private)\\
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Face classification} --- The detected face region is expanded to
        ...
        (1.2\,ms/face, GPU).
  \item \textbf{Text field detection} --- PaddleOCR PP-OCRv6 detects all text
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}
    \textbf{[PLACEHOLDER: Pipeline diagram]}\\[0.5em]
    Image $\rightarrow$ Face detect $\rightarrow$ Face classify $\rightarrow$ photo\_prob\\
    Image $\rightarrow$ PaddleOCR $\rightarrow$ Field crops $\rightarrow$ Forensic panels $\rightarrow$ ConvNeXt-B $\rightarrow$ text\_prob\\
    fraud\_score = max(photo\_prob, text\_prob)
  \vspace{1cm}}}
...
We use MediaPipe BlazeFace for face detection, selecting the largest left-side
...
The portrait crop is classified by a DINOv2-small backbone
...
\begin{figure}[H]
  ...
  % \includegraphics[width=\textwidth]{figures/face_detection_examples.pdf}
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{4cm}
    \textbf{[PLACEHOLDER: Face detection examples]}\\[0.5em]
    Show 4--6 documents with detected face box (green), portrait crop (blue),
    and predicted label (real/fake) for each country type.
  \vspace{1cm}}}
...
\begin{figure}[H]
  ...
  % \includegraphics[width=\textwidth]{figures/forensic_panel_example.pdf}
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{5cm}
    \textbf{[PLACEHOLDER: Forensic panel examples]}\\[0.5em]
    Show 2--3 panels side by side:\\
    (a) genuine field panel --- uniform views, no anomalies\\
    (b) tampered field panel --- visible artifacts in ink mask, L residual, edge views\\
    Label each of the 6 views.
  \vspace{1cm}}}
...
The forensic panels are classified by a ConvNeXt-Base backbone initialized from
\texttt{facebook/dinov3-convnext-base-pretrain-lvd1689m} (88M parameters).
...
Linear$(1024, 256)$ $\rightarrow$ GELU $\rightarrow$ Dropout$(0.15)$ $\rightarrow$
Linear$(256, 1)$.
...
\begin{figure}[H]
  ...
  % \includegraphics[width=\textwidth]{figures/synth_effects_grid.pdf}
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{5cm}
    \textbf{[PLACEHOLDER: Synthetic tamper effect examples]}\\[0.5em]
    3$\times$3 grid showing each of the 9 effects applied to the same field crop.
    Label each effect name. Show original clean crop in the center or as header.
  \vspace{1cm}}}
...
\begin{figure}[H]
  ...
  % \includegraphics[width=\textwidth]{figures/inference_examples.pdf}
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{5cm}
    \textbf{[PLACEHOLDER: Inference prediction examples]}\\[0.5em]
    Show 4--6 test documents with:\\
    - Face detection box + photo\_prob score\\
    - PaddleOCR field boxes colored by field\_prob (green=low, red=high)\\
    - Final fraud\_score\\
    Include both correct predictions and failure cases.
  \vspace{1cm}}}
...
All model weights and PaddleOCR detection models are baked into the image
(no runtime downloads).
HuggingFace model configs are pre-cached at build time; inference runs with
\texttt{TRANSFORMERS\_OFFLINE=1}.
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.8\textwidth}{\centering\vspace{4cm}
    ...
    Plot: loss, panel AUC, doc AUC, FREUID per epoch.
    Annotate epoch 2 as selected checkpoint.
  \vspace{1cm}}}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Weights:} \url{https://huggingface.co/akhil838/doc-forensic-models-v1}
        ...
        PaddleOCR 59\,MB).
  \item \textbf{Docker:} \texttt{docker build --build-arg HF\_TOKEN=... -t freuid-repro .}
...
\begin{figure}[H]
  ...
  % \includegraphics[width=\textwidth]{figures/field_heatmap.pdf}
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{5cm}
    \textbf{[PLACEHOLDER: Per-field prediction heatmap]}\\[0.5em]
    Show a document image with each PaddleOCR field box overlaid,
    colored by the ConvNeXt-B probability (green$\rightarrow$red colormap).
    Show one genuine document (all green) and one tampered document
    (one red field among green fields).
  \vspace{1cm}}}

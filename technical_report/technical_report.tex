...
inference runs in approximately 6 hours on an NVIDIA A100 for 134{,}997 documents.
...
\hspace*{1.5cm}(commit \texttt{dea74aa68})\\
...
\begin{table}[H]
  \centering
  \small
  \caption{Inference timing for 134{,}997 private test documents (A100-SXM4-40GB).}
  \label{tab:timing}
  \begin{tabular}{lrrr}
    \toprule
    Step & Time & Rate & Notes \\
    \midrule
    MediaPipe face detection    & 0.06\,h &  604 img/s & 16 threads, CPU \\
    DINOv2-small face classify  & 0.12\,h &  2.8 batch/s & batch=128, FP16 \\
    PaddleOCR text detection    & 1.5\,h  &   24 img/s & single GPU process \\
    ConvNeXt-B panel scoring    & 4.3\,h  &  128 panels/s & torch.compile, FP16 \\
    \midrule
    \textbf{Total}              & \textbf{$\sim$6\,h} & & 1.99M panels (15 fields/doc cap) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Dependencies:} Python 3.12, PyTorch 2.7+cu129, PaddlePaddle 3.3,
  ...
  \item \textbf{Inference hardware:} NVIDIA A100-SXM4-40GB, 16-core CPU,
        120\,GB RAM, 130\,GB disk (Vast.ai).
        Wall-clock: $\sim$6 hours for 134{,}997 documents (1.99M forensic panels).
